...
/// Convert a display name into a similar-enough username.
fn usernameify(display_name: &str) -> String {
    display_name
        .chars()
        .filter(|c| c.is_alphanumeric())
        .map(|c| c.to_ascii_lowercase())
        .skip_while(|c| c.is_numeric()) // No leading numbers
        .collect()
}
...
pub fn Register() -> impl IntoView {
    ...
    let display_name = RwSignal::new("".to_string());
    let username = RwSignal::new("".to_string());
    let bio = RwSignal::new("".to_string());

    // Whether the username has been modified from the autogenerated suggestion.
    let username_dirty = RwSignal::new(false);
                ...
                >
                    ...
                    <div class="py-2">
                        ...
                        <input
                            ...
                            on:input:target=move |ev| {
                                if !username_dirty() {
                                    username.set(usernameify(&ev.target().value()));
                                }
                            }
                        />
                        <p>
                    ...
                    <div class="py-2">
                        ...
                        <input
                            ...
                            bind:value=username
                            on:input:target=move |ev| {
                                username_dirty.set(!ev.target().value().is_empty());
                            }
                        />
                        <p>
